...
class SamplePresenter(private var sampleView: SampleView?) {
    ...
    fun onLocationFailed(failType: FailType) {
        sampleView!!.dismissProgress()
        when (failType) {
            FailType.TIMEOUT -> {
            ...
            FailType.GOOGLE_PLAY_SERVICES_NOT_AVAILABLE -> {
                sampleView!!.text =
                    "Couldn't get location, because Google Play Services not available!"
            }
            ...
            FailType.GOOGLE_PLAY_SERVICES_SETTINGS_DENIED -> {
                sampleView!!.text =
                    "Couldn't get location, because user didn't activate providers via settingsApi!"
            }
            FailType.VIEW_DETACHED -> {
                sampleView!!.text =
                    "Couldn't get location, because in the process view was detached!"
            }
    ...
    fun onProcessTypeChanged(newProcess: ProcessType) {
        when (newProcess) {
