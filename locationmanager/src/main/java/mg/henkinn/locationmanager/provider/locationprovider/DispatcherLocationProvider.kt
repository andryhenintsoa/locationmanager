...
open class DispatcherLocationProvider() : LocationProvider(),
    ContinuousTaskRunner, FallbackListener {
    private var gpServicesDialog: Dialog? = null
    private var activeProvider: LocationProvider? = null
    ...
    override val isDialogShowing: Boolean
        get() {
            val gpServicesDialogShown = gpServicesDialog?.isShowing ?: false
            val anyProviderDialogShown = activeProvider?.isDialogShowing ?: false
    ...
    override fun runScheduledTask(taskId: String) {
        if (taskId == DispatcherLocationSource.GOOGLE_PLAY_SERVICE_SWITCH_TASK) {
            if (activeProvider is GooglePlayServicesLocationProvider && activeProvider!!.isWaiting) {
                logI("We couldn't receive location from GooglePlayServices, so switching default providers...")
                cancel()
    ...
    override fun get() {
        if (getConfiguration()!!.googlePlayServicesConfiguration() != null) {
            ...
            logI(
                "Configuration requires not to use Google Play Services, "
    ...
    open fun checkGooglePlayServicesAvailability(askForGooglePlayServices: Boolean) {
        val gpServicesAvailability: Int = sourceProvider.isGoogleApiAvailable(context)
        if (gpServicesAvailability == ConnectionResult.SUCCESS) {
            logI("GooglePlayServices is available on device.")
            getLocationFromGooglePlayServices()
        } else {
            logI("GooglePlayServices is NOT available on device.")
            if (askForGooglePlayServices) {
                askForGooglePlayServices(gpServicesAvailability)
            } else {
                logI(
                    ("GooglePlayServices is NOT available and even though we ask user to handle error, "
    ...
    open fun askForGooglePlayServices(gpServicesAvailability: Int) {
        if (getConfiguration()!!.googlePlayServicesConfiguration()!!.askForGooglePlayServices() &&
            ...
            logI(
                ("Either GooglePlayServices error is not resolvable "
        ...
        if (gpServicesDialog != null) {
            ...
            when (gpServicesAvailability) {
                ConnectionResult.SERVICE_INVALID, ConnectionResult.SERVICE_UPDATING -> gpServicesDialog!!.setOnDismissListener {
                    logI("GooglePlayServices error could not have been resolved")
                    continueWithDefaultProviders()
                }
            }
            ...
            logI(
                ("GooglePlayServices error could've been resolved, but since LocationManager "
    ...
    open fun getLocationFromGooglePlayServices() {
        logI("Attempting to get location from Google Play Services providers...")
        setLocationProvider(sourceProvider.createGooglePlayServicesLocationProvider(this))
        sourceProvider.gpServicesSwitchTask().delayed(
            getConfiguration()!!.googlePlayServicesConfiguration()!!
                .googlePlayServicesWaitPeriod()
        )
        activeProvider!!.get()
    }
     ...
     */
    open fun continueWithDefaultProviders() {
        if (getConfiguration()!!.defaultProviderConfiguration() == null) {
            logI("Configuration requires not to use default providers, abort!")
            listener?.onLocationFailed(FailType.GOOGLE_PLAY_SERVICES_NOT_AVAILABLE)
        } else {
            logI("Attempting to get location from default providers...")
    ...
    // For test purposes
    internal fun setDispatcherLocationSource(dispatcherLocationSource: DispatcherLocationSource) {
