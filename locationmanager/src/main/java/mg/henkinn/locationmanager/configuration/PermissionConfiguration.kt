...
open class PermissionConfiguration private constructor(builder: Builder) {
    private val permissionProvider: PermissionProvider?

    init {
        permissionProvider = builder.permissionProvider
    }

    open fun permissionProvider(): PermissionProvider? {
        return permissionProvider
    }

    class Builder {
        private var rationaleMessage = Defaults.EMPTY_STRING
        ...
        fun requiredPermissions(permissions: Array<String>?): Builder {
            require(!(permissions == null || permissions.size == 0)) { "requiredPermissions cannot be empty." }
        ...
        fun build(): PermissionConfiguration {
            if (rationaleDialogProvider == null && rationaleMessage.isNotEmpty()) {
                rationaleDialogProvider = SimpleMessageDialogProvider(rationaleMessage)
            }
            if (permissionProvider == null) {
                permissionProvider =
                    DefaultPermissionProvider(requiredPermissions, rationaleDialogProvider)
            }
            return PermissionConfiguration(this)
        }
    }
}