...
import mg.henkinn.locationmanager.configuration.Defaults.createDefaultLocationRequest


open class GooglePlayServicesConfiguration private constructor(builder: Builder) {
    ...
    // region Getters
    open fun locationRequest(): LocationRequest {
        return locationRequest
    }

    open fun fallbackToDefault(): Boolean {
        return fallbackToDefault
    }

    open fun askForGooglePlayServices(): Boolean {
        return askForGooglePlayServices
    }

    open fun askForSettingsApi(): Boolean {
        return askForSettingsApi
    }

    open fun failOnSettingsApiSuspended(): Boolean {
        return failOnSettingsApiSuspended
    }

    open fun ignoreLastKnowLocation(): Boolean {
        return ignoreLastKnowLocation
    }

    open fun googlePlayServicesWaitPeriod(): Long {
        return googlePlayServicesWaitPeriod
    }

    // endregion
    class Builder {
        internal var locationRequest = createDefaultLocationRequest()
        internal var fallbackToDefault = Defaults.FALLBACK_TO_DEFAULT
        internal var askForGooglePlayServices = Defaults.ASK_FOR_GP_SERVICES
        internal var askForSettingsApi = Defaults.ASK_FOR_SETTINGS_API
        internal var failOnSettingsApiSuspended = Defaults.FAIL_ON_SETTINGS_API_SUSPENDED
        internal var ignoreLastKnowLocation = Defaults.IGNORE_LAST_KNOW_LOCATION
        internal var googlePlayServicesWaitPeriod = Defaults.WAIT_PERIOD
